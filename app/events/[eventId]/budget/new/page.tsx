...
import { getBudgetOverview, getPreviousBudgetContext } from "@/lib/budget/data";
...
export default async function NewBudgetPage({
  ...
  const [budget, previous] = await Promise.all([
    getBudgetOverview(session.supabase, eventId),
    getPreviousBudgetContext(session.supabase, eventId),
  ]);
  ...
  return <BudgetEditor eventId={eventId} departments={budget.data.departments.filter((department) => department.is_active)} previousBudget={previous.data} error={query.error} />;
}
