...
import { OtherRevenuePanel } from "@/components/revenue-panel";
...
export default async function OtherRevenuePage({
  ...
  const session = await getAuthenticatedSession(`/events/${eventId}/revenue/other`);
  ...
  if (revenue.error || !revenue.data) {
    return <div role="alert" className="rounded-md border border-destructive/40 p-4 text-sm text-destructive">{revenue.error ?? "Other revenue could not be loaded."}</div>;
  }
  ...
  return (
    <OtherRevenuePanel
      eventId={eventId}
      items={revenue.data.otherItems}
      owners={revenue.data.owners}
