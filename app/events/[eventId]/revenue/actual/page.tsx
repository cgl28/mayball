...
import { TicketSnapshotsPanel } from "@/components/revenue-panel";
...
export default async function ActualTicketRevenuePage({
  ...
  const session = await getAuthenticatedSession(`/events/${eventId}/revenue/actual`);
  ...
  if (revenue.error || !revenue.data) {
    return <div role="alert" className="rounded-md border border-destructive/40 p-4 text-sm text-destructive">{revenue.error ?? "Ticket snapshots could not be loaded."}</div>;
  }
  ...
  return (
    <TicketSnapshotsPanel
      eventId={eventId}
      snapshots={revenue.data.snapshots}
      breakdowns={revenue.data.breakdowns}
