...
select results_eq($$select count(*)::bigint from public.events$$,$$values(2::bigint)$$,'member sees current and historical events only');
...
select results_eq($$select count(*)::bigint from public.events$$,$$values(1::bigint)$$,'outsider sees only own event');
select results_eq($$select count(*)::bigint from public.departments$$,$$values(0::bigint)$$,'outsider sees no Downing departments');
select results_eq($$select count(*)::bigint from public.ticket_sales_snapshots$$,$$values(0::bigint)$$,'outsider sees no Downing revenue');
